...
1. Fall back to `~/.config/opencode/oh-my-opencode-slim/`.
1. If both `{agent}.md` and `{agent}_append.md` exist, the replacement prompt is
   loaded first and the append file is added after it.
1. If no prompt files exist, the built-in prompt from the plugin package is used.
...
1. **Avoid hidden behavior changes.**
...
1. **Tell the user about restart requirements.**
...
1. **Decide the smallest useful change**
