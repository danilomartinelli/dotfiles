...
type NotificationCommandRuntime = AlerterRuntime;
...
export async function sendMacOSAlerterNotification(
	...
	try {
		...
		warn(
			`notify: macOS desktop notification skipped; alerter exited with code ${exitCode}.`,
		);
		return false;
	} catch (error) {
...
export async function sendLinuxNotifySendNotification(
	options: DesktopNotificationOptions,
	runtime: NotificationCommandRuntime = {},
): Promise<boolean> {
	const which = runtime.which ?? Bun.which;
	const warn = runtime.warn ?? console.warn;

	try {
		const notifySendPath = await which("notify-send");
		if (!notifySendPath) {
			warn(
				"notify: Linux desktop notification skipped; notify-send is not available on PATH.",
			);
			return false;
		}
		const summary = options.subtitle
		...
		const spawnProcess =
			...
			((argv: string[]) =>
				Bun.spawn(argv, { stdout: "ignore", stderr: "pipe" }));
		const process = spawnProcess([notifySendPath, summary, options.message]);
		const exitCode = await process.exited;
		if (exitCode === 0) return true;
		warn(
			`notify: Linux desktop notification skipped; notify-send exited with code ${exitCode}.`,
		);
		return false;
	} catch (error) {
		const message = error instanceof Error ? error.message : String(error);
		warn(
			`notify: Linux desktop notification skipped; notify-send failed (${message}).`,
		);
		return false;
	}
}
...
export async function sendDesktopNotificationByPlatform(
	...
	if (platform === "linux") {
		await (sendLinuxNotification ?? sendLinuxNotifySendNotification)(
			notificationOptions,
		);
		return;
	}
	...
	warn(
		`notify: desktop notifications are not configured for platform ${platform}.`,
	);
}
