...
/** Supported input exposed by the installed experimental system transform hook. */
 ...
 *
 * Provides plan management and universal date awareness.
...
// Agent-specific routing rules are configured on the relevant agents.
...
const WorkspacePlugin: Plugin = async (ctx) => {
	...
	return {
		...
		// The SDK hook input does not identify the active agent. Agent-specific
		// routing and worktree rules live in the corresponding agent config.
