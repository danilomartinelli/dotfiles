...
| Pattern | Purpose | Example | Destination |
|---------|---------|---------|-------------|
| `*.symlink` | Files to link to home directory | `gitconfig.symlink` | `~/.gitconfig` |
| `path.zsh` | PATH modifications (loaded first) | Add `/usr/local/bin` | Shell PATH |
| `*.zsh` | Main shell configuration | Aliases, functions | Shell environment |
| `completion.zsh` | Shell completions (loaded last) | Command completions | Zsh completion system |
| `install.sh` | Installation scripts | Install packages | System setup |
| `aliases.zsh` | Command aliases | `alias ll='ls -la'` | Shell aliases |
